...
const char printTopic[]  = "arduino/prints";



ScanerCore::ScanerCore(MqttClient globalMqttClient) : mqttClient{ globalMqttClient }{}
...
void ScanerCore::StartMessage() {
  switch(currentPrinter) {
    ...
    case mqtt : {
      mqttClient.beginMessage(printTopic);
      break;
    }
  }
}
...
void ScanerCore::MeasureBuoyancyValues(int averages) {  
  for (int i = 0; i < averages; i++) {
    PrintMessage(String(MeasureADC()));
  }
  PrintMessage("");
}
